...
export default class FilmList {
  ...
  init(data) {
    ...
    render(mainNode, this._sectionFilm, RENDER_POSITION.BEFOREEND);
    this.filmContainerBlockNode = this._container.querySelector(`.films-list__container`);
  ...
  _renderFilm() {
    if (this._data.length > FILMS_COUNT_PER_STEP) {
      for (let i = 0; i < FILMS_COUNT_PER_STEP; i++) {
        const film = this._data.splice(0, 1);
        const filmCardElement = new FilmCard(film[0]);
        render(this.filmContainerBlockNode, filmCardElement, RENDER_POSITION.BEFOREEND);
      }
    } else {
  ...
  _renderMostCommentsFilms() {
    ...
    const topRateArray = this._sourcedData.slice();
    sortMostComments(topRateArray);
    const topRateContainer = this._container.querySelectorAll(`.films-list--extra .films-list__container`);
    for (let i = 0; i < FILMS_COUNT_IN_EXTRA; i++) {
      const filmCardElement = new FilmCard(topRateArray[i]);
      render(topRateContainer[1], filmCardElement, RENDER_POSITION.BEFOREEND);
    }
